...
def create_document(org_id:UUID, workflow_id:UUID, doc_type_id:UUID,document:Document):
    try:
...
def list_documents(org_id: UUID, workflow_id: UUID):
    with get_session() as session:
...
def get_document(org_id: UUID, workflow_id: UUID, document_id: UUID):
    with get_session() as session:
...
def update_document(org_id: UUID, workflow_id: UUID, document_id: UUID, updated_doc: Document):
    with get_session() as session:
        document = session.get(Document, document_id)
        if not document:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Document not found")
...
def delete_document(org_id: UUID, workflow_id: UUID, document_id: UUID):
    try:
        with get_session() as session:
            document = session.get(Document, document_id)
            if not document:
