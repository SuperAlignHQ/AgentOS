...
from schemas.AuthenticationSchema import RegisterRequest,LoginRequest

router = APIRouter()


@router.post("/register", response_model=User)
def register_user(user:RegisterRequest,session:Session=Depends(get_session)):
...
@router.post("/login")
def login_user(user1:LoginRequest,session:Session=Depends(get_session)):
